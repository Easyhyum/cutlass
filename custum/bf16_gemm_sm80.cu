...
torch::Tensor run_gemm(GemmOp& op, torch::Tensor A, torch::Tensor B,
    ...
    set_sleep_params(sleep_ns, sleep_freq);
...
torch::Tensor gemm_sm80_v2(torch::Tensor A, torch::Tensor B,
    ...
    return run_gemm(gemm_op_256x128, A, B, "256x128", sleep_ns, sleep_freq);
}
torch::Tensor gemm_sm80_v3(torch::Tensor A, torch::Tensor B,
                            unsigned int sleep_ns = 0u, unsigned int sleep_freq = 1u) {
    return run_gemm(gemm_op_128x128k64, A, B, "128x128k64", sleep_ns, sleep_freq);
}
torch::Tensor gemm_sm80_v4(torch::Tensor A, torch::Tensor B,
                            unsigned int sleep_ns = 0u, unsigned int sleep_freq = 1u) {
    return run_gemm(gemm_op_128x256s5, A, B, "128x256s5", sleep_ns, sleep_freq);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("gemm_sm80",    &gemm_sm80,    "BF16 128×256×32 3stage");
    m.def("gemm_sm80_v2", &gemm_sm80_v2, "BF16 256×128×32 3stage",
          py::arg("A"), py::arg("B"),
          py::arg("sleep_ns")=0u, py::arg("sleep_freq")=1u);
    m.def("gemm_sm80_v3", &gemm_sm80_v3, "BF16 128×128×64 3stage",
          py::arg("A"), py::arg("B"),
          py::arg("sleep_ns")=0u, py::arg("sleep_freq")=1u);
    m.def("gemm_sm80_v4", &gemm_sm80_v4, "BF16 128×256×32 5stage",
          py::arg("A"), py::arg("B"),
          py::arg("sleep_ns")=0u, py::arg("sleep_freq")=1u);
}
