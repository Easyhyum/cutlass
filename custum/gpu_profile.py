...
def _monitor_process_worker(device_id, metrics_queue, stop_event, shutdown_event):
    """별도 프로세스에서 GPU 메트릭 수집"""
    try:
        # 프로세스 내에서 NVML 재초기화
        pynvml.nvmlInit()
        handle = pynvml.nvmlDeviceGetHandleByIndex(device_id)
        print(f"GPU 모니터링 프로세스 시작 (PID: {os.getpid()}, GPU: {device_id})")
        ...
        while not shutdown_event.is_set():
            if not stop_event.is_set():
                ...
                # Idle mode: keep process alive but do not collect metrics.
                pass
...
class GPUMonitor:
    def __init__(self, device_id=0):
        ...
        if NVML_AVAILABLE:
            ...
            # 모니터링 프로세스 시작 (모듈 레벨 함수 사용)
            self.monitor_proc = Process(
                target=_monitor_process_worker,
                args=(self.device_id, self.metrics_queue, self.stop_event,
                      self.shutdown_event),
            )
            self.monitor_proc.start()
            # Start in idle mode; start_monitoring() enables collection.
            self.stop_event.set()
            # monitor_proc가 start_time을 metrics_queue에 보낼 때까지 대기
            latency_for_start_time = time.time()
            start_time = None
            try:
                msg_type, data = self.metrics_queue.get(timeout=10.0)
                if msg_type == 'start_time':
                    start_time = data
            except:
                pass
            during_time = time.time() - latency_for_start_time
            print(f"모니터링 시작 대기 시간: {during_time:.2f} 초, start_time={start_time}")
    ...
    def start_monitoring(self):
        ...
        # metric queue를 모두 비운다.
        while not self.metrics_queue.empty():
            try:
                _, _ = self.metrics_queue.get_nowait()
            except:
                break
        """모니터링 시작"""
        self.metrics = []
    ...
    def stop_monitoring(self):
        ...
        if self.metrics_queue:
            while not self.metrics_queue.empty():
                try:
                    msg_type, data = self.metrics_queue.get_nowait()
                    if msg_type == 'metric':
    ...
    def close(self):
        ...
        if self.monitor_proc is not None:
            self.monitor_proc.join(timeout=1.0)
    ...
    def collect_during_execution(self, func, *args, num_samples=None, **kwargs):
        ...
        self.metrics = []
        self.num_samples = (num_samples * self.repeat_count) if self.repeat_count else num_samples
        ...
        # 모니터링 프로세스 시작 (모듈 레벨 함수 사용)
        monitor_proc = Process(target=_monitor_process_worker, args=(self.device_id, metrics_queue, stop_event))
        monitor_proc.start()
        
        # start_time 받기
        start_time = None
        try:
            msg_type, data = metrics_queue.get(timeout=1.0)
            if msg_type == 'start_time':
                start_time = data
        except:
            pass
    ...
    def update_environment_variables(envs_dict: dict[str, str]):
        ...
        for k, v in envs_dict.items():
            ...
            os.environ[k] = v
    def save_statistics_to_csv(self, path=csv_path, during_time=None, decoding_steps=None, kv_cache_lens=None, batch_size=None, graph_batch_size=None, cudagraph_mode=None, manipulated=None, model_mode=None, repeat_count=None, input_len = None, model_name=None, sm_clock=None, iteration=None, sleep_ns=None, sleep_freq=None, kernel=None):
        """수집된 메트릭을 CSV 파일로 저장"""
        if not self.metrics:
            print("저장할 메트릭 데이터가 없습니다.")
            return
        self.num_samples = (batch_size * repeat_count) if repeat_count else batch_size
        import csv
        ...
        graph_mode = None
        if os.environ.get("ALL_GRAPH_BATCH") == "1":
            graph_mode = "all"
        elif os.environ.get("GRAPH_MANIPULATION") == "1":
            graph_mode = "mani"
        else:
            graph_mode = "seg"
        keys = ['batch_size', 'during_time', 'index', 'length', 
                ...
                'total_energy', 'repeat_count', 'input_len','target_sm_clock', 'iteration', 
                'sleep_ns', 'sleep_freq', 'kernel']
    
        pid = os.environ.get("VLLM_WMMA_TEST_PID", str(os.getpid()))
        file_path = os.path.join(path, f"gpu_profile_{pid}.csv")
            ...
            with open(file_path, mode='a', newline='') as csvfile:
                ...
                for i, metric in enumerate(self.metrics):
                    ...
                    metric_row.update({
                        ...
                        "during_time": f"{during_time:.4f}" if during_time else None, 
                        "length": length, 
                        "index": i + 1,
                        "avg_power_js": f"{avg_power_js:.4f}" if avg_power_js else None,
                        "total_energy_j": total_energy,
                        "energy_per_sample": f"{energy_per_sample:.6f}" if energy_per_sample else None,
                        "throughput": f"{throughput:.4f}" if throughput else None,
