...
#
# Usage:
#   ./watch_plot.sh                  # plot the latest logs/tchunk_*/  every 60s
#   ./watch_plot.sh logs/tchunk_XYZ  # plot a specific tag dir every 60s
...
if [ -n "${1:-}" ]; then
    ...
    TAG_DIR=$(ls -d logs/tchunk_* 2>/dev/null | tail -1 || true)
fi
