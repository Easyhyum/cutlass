...
#
# Usage:
#   ./watch_plot.sh                  # plot the latest logs/tchunk_ol_*/  every 60s
#   ./watch_plot.sh logs/tchunk_ol_XYZ  # plot a specific tag dir every 60s
...
if [ -n "${1:-}" ]; then
    ...
    TAG_DIR=$(ls -d logs/tchunk_ol_* 2>/dev/null | tail -1 || true)
fi
