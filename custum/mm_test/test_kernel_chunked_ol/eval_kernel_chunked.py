...
def load_kernel(kernel):
    ...
    if kernel == 'streamk':
        sys.path.insert(0, os.path.join(HERE, 'build_streamk_chunked_ol'))
        import bf16_gemm_sm80_streamk_chunked_ol as ext
        # _ol uses gemm_streamk_chunked_ol(A, B, chunk_m, split_k, avail_sms, n_streams)
        # `idle` param now reused as n_streams (default 2 if 0 passed).
        def fn_chunked(A, B, cm, idle):
            n_streams = idle if idle > 0 else 2
            return ext.gemm_streamk_chunked_ol(A, B, cm, 1, -1, n_streams)
        return (ext, fn_chunked, (lambda A, B: ext.gemm_streamk(A, B)))
    if kernel == 'sm80_v3':
        sys.path.insert(0, os.path.join(HERE, 'build_sm80_v3_chunked_ol'))
        import bf16_gemm_sm80_v3_chunked_ol as ext
        def fn_chunked(A, B, cm, idle):
            n_streams = idle if idle > 0 else 2
            return ext.gemm_sm80_v3_chunked_ol(A, B, cm, n_streams)
        return (ext, fn_chunked, (lambda A, B: ext.gemm_sm80_v3(A, B)))
