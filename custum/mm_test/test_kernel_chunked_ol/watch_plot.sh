...
#
# Usage:
#   ./watch_plot.sh                  # plot the latest logs/kchunk_ol_*/  every 60s
#   ./watch_plot.sh logs/kchunk_ol_XYZ  # plot a specific tag dir every 60s
...
if [ -n "${1:-}" ]; then
    ...
    TAG_DIR=$(ls -d logs/kchunk_ol_* 2>/dev/null | tail -1 || true)
fi
