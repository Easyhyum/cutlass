#!/usr/bin/env bash
# Qwen3-8B per-call timing eval on GPU 0.
...
echo "[run_qwen3_8b_timing] GPU=$GPU  TAG=$TAG"
echo "  log      : $RUN_LOG"
...
echo
echo "[run_qwen3_8b_timing] rendering plot + table ..."
python3 plot_percall_time.py \
    --segments "$SEG_CSV" \
    --out-png  "$LOG_DIR/${TAG}_percall.png" \
    --out-md   "$LOG_DIR/${TAG}_table.md" \
    --title    "Qwen3-8B per-MatMul time  (50 bursts, GPU 0)"

echo
echo "[run_qwen3_8b_timing] DONE"
echo "  segments : $SEG_CSV"
