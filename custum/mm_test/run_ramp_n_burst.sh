...
# 50ms nvidia-smi sampling (HW rate is ~100ms, oversample acceptable)
nvidia-smi -i "$GPU" \
...
echo "[run_ramp_n_burst] GPU=$GPU"
python3 -u eval_ramp_n_burst.py 2>&1 | tee "$RUN_LOG"
...
echo
echo "[run_ramp_n_burst] enriching..."
python3 analyze_power.py "$SEG_CSV" "$POWER_CSV" \
    --threshold 660 --drop-tol 50 \
    2>&1 > "$LOG_DIR/${TAG}_analysis.txt" || true

echo
echo "[run_ramp_n_burst] aggregating per-config across bursts..."
python3 - <<PY
import csv
...
# baseline = best s100_* mean
def mean(xs): return sum(xs)/len(xs)
def stdv(xs): m=mean(xs); return (sum((x-m)**2 for x in xs)/max(len(xs)-1,1))**0.5

base_rows = {k: v for k,v in groups.items() if k.startswith('s100')}
base_key = max(base_rows.keys(), key=lambda k: mean([float(r['tflops']) for r in base_rows[k]]))
base = base_rows[base_key]
...
print(f"{'cfg':>10s} {'N':>3s}  {'TFLOPS':>7s} {'%base':>5s}  "
      f"{'max_W μ':>8s} {'σ':>5s} {'Δμ':>6s}  "
