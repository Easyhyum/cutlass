...
CONFIG_DEFS = {
    ...
    # BM-* (v6 current): warp-MIS-aligned bubble sync.
    # Sleep is placed INSIDE warp_mma_k loop, gated by
    #   warp_mma_k == (warp_in_cta & (kWarpGemmIterations-1))
    # → at any instant, only HALF the warps sleep, other half continues to
    #   the next HMMA / copy_tiles → CTA progress not bound by sleep.
    # Format: BM-<ns>-<mod>
    #   ns = nanosleep duration; mod just gates ON/OFF (>1 = enabled).
    # Math (down_proj M=8192 K=12288, kWarpGemmIterations=2, 384 outer iters):
    #   Each warp sleeps 1× per outer iter (at its assigned warp_mma_k slot).
    #   Critical path per outer iter ≈ stagger_ns (one sleep happens on path)
    #   Total per kernel ≈ stagger_ns × 384.
    #   BM-50-2  : 50ns × 384 = 19us per kernel → 0.16% theoretical
    #   BM-100-2 : 100ns × 384 = 38us → 0.32%
    #   BM-200-2 : 200ns × 384 = 77us → 0.64%
    #   BM-500-2 : 500ns × 384 = 192us → 1.6%
    'BM-25-2':      (0,  1,   25,  2),
    'BM-50-2':      (0,  1,   50,  2),
    'BM-100-2':     (0,  1,  100,  2),
    'BM-200-2':     (0,  1,  200,  2),
    'BM-500-2':     (0,  1,  500,  2),
    'BM-1000-2':    (0,  1, 1000,  2),
    'BM-2000-2':    (0,  1, 2000,  2),
