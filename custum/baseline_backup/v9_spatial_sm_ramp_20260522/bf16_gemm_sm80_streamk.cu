...
// v8 (A) linear-activity ramp model.
//   sleep_ns/sleep_freq 인자는 API 하위 호환 (무시).
//   ramp_start_pct : activity %/100 at iter 0 (100 = ramp OFF)
//   ramp_step_pct  : activity % increase per outer iter
//   ramp_iter_time_ns : host estimate of mainloop outer iter time (ns)
...
torch::Tensor gemm_streamk(torch::Tensor A, torch::Tensor B,
    ...
    if (g_oneshot_ramp_armed) {
        ...
        g_oneshot_ramp_armed = false;  // consume
    }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("prime_ramp", &prime_ramp,
          "Prime a one-shot linear-activity ramp for the NEXT gemm_streamk "
          "call. Auto-resets after that single call.",
