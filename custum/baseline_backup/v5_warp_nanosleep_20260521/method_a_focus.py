...
CONFIG_DEFS = {
    ...
    # BW-* (v5): warp-staggered __nanosleep bubble sync.
    # Fires INSIDE mac_loop_iter, AFTER gmem_wait() — like PW but:
    #   - phase = (warp_in_cta + blockIdx phase) % mod  (no smid PTX lookup)
    #   - __nanosleep instead of clock64 busy-spin (warp yields → SM idle, low power)
    # Format: BW-<ns>-<mod>
    #   ns is now in NANOSECONDS (real nanosleep duration), not cycles.
    # Math for down_proj M=8192 K=12288 (384 outer iters per kernel, 12ms total):
    #   worst phase per iter sleeps (mod-1) * stagger_ns
    #   BW-10-8  : 7*10 = 70ns * 384 = ~27us per kernel → 0.22% theoretical overhead
    #   BW-50-8  : 7*50 = 350ns * 384 = ~134us per kernel → 1.1%
    #   BW-100-8 : 7*100 = 700ns * 384 = ~269us → 2.2%
    #   BW-200-8 : 7*200 = 1400ns * 384 = ~538us → 4.5%
    # NOTE: nanosleep has ~50ns hardware minimum; BW-10-* may not differ much from BW-50-*.
    'BW-10-8':      (0,  1,   10,  8),
    'BW-25-8':      (0,  1,   25,  8),
    'BW-50-8':      (0,  1,   50,  8),
    'BW-100-8':     (0,  1,  100,  8),
    ...
    'BW-500-8':     (0,  1,  500,  8),
    'BW-50-16':     (0,  1,   50, 16),
    'BW-100-16':    (0,  1,  100, 16),
    'BW-200-16':    (0,  1,  200, 16),
