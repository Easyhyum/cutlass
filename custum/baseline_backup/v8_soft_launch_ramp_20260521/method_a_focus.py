...
CONFIG_DEFS = {
    ...
    # peak_ns = initial sleep at outer_iter=0 (linearly decreasing)
    # iters   = ramp duration in outer iters (kernel has ~384 outer iters for
    #           down_proj M=8192; ramp 50~100 iters = ~13~26% of kernel start)
    # Cost (worst case, average sleep = peak/2 across ramp):
    #   RAMP-500-i50  : avg 250ns * 50 = 12.5us/kernel → 0.1%
    #   RAMP-1000-i50 : 25us/kernel → 0.21%
    #   RAMP-2000-i100: 100us/kernel → 0.83%
    #   RAMP-5000-i100: 250us/kernel → 2.08%
    'RAMP-500-i50':    _cfg(ramp_peak_ns=500,  ramp_iters=50),
    'RAMP-1000-i50':   _cfg(ramp_peak_ns=1000, ramp_iters=50),
    'RAMP-2000-i50':   _cfg(ramp_peak_ns=2000, ramp_iters=50),
    'RAMP-1000-i100':  _cfg(ramp_peak_ns=1000, ramp_iters=100),
    'RAMP-2000-i100':  _cfg(ramp_peak_ns=2000, ramp_iters=100),
    'RAMP-5000-i100':  _cfg(ramp_peak_ns=5000, ramp_iters=100),
    'RAMP-10000-i100': _cfg(ramp_peak_ns=10000, ramp_iters=100),
}
