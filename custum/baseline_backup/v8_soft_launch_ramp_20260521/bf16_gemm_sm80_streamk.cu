...
// v8: stagger params (v7 BM-*) + ramp params (v8 soft-launch).
// sleep_ns/sleep_freq 인자는 API 하위 호환 (무시).
static void set_sleep_params(unsigned int sleep_ns, unsigned int sleep_freq,
                             ...
                             unsigned int ramp_peak_ns = 0u,
                             unsigned int ramp_iters   = 1u) {
    ...
    cudaMemcpyToSymbol(kCutlassRampPeakNs,      &ramp_peak_ns, sizeof(unsigned int));
    cudaMemcpyToSymbol(kCutlassRampIters,       &ramp_iters,   sizeof(unsigned int));
#else
    (void)stagger_ns; (void)stagger_mod;
    (void)ramp_peak_ns; (void)ramp_iters;
#endif
}
...
torch::Tensor gemm_streamk(torch::Tensor A, torch::Tensor B,
                           ...
                           unsigned int ramp_peak_ns = 0u, unsigned int ramp_iters = 1u) {
    ...
    // Method A: upload sleep + ramp params before kernel launch
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("gemm_streamk", &gemm_streamk,
          ...
          py::arg("ramp_peak_ns")   = 0u,
          py::arg("ramp_iters")     = 1u);
