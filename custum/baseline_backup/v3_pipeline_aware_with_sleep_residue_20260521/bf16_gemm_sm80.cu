...
// host 측 setter (커널 런치 직전에 호출)
//
// Method A semantics:
//   - sleep_ns   : 모든 SM 공통 base sleep (ns), 매 sleep_freq outer iter 마다 1회
//   - sleep_freq : 위 base sleep 적용 주기 (1=매 iter, kStages 보다 큰 값 권장)
//   - stagger_ns : SM phase 당 추가 sleep (ns). prologue 1회 + 위 주기에서 추가
//   - stagger_mod: SM phase 개수 (2 이상). smid % stagger_mod 가 phase ID
//
//   sleep_ns=0 && stagger_ns=0  → baseline (sleep 호출 없음)
//   sleep_ns=0 && stagger_ns>0  → 순수 SM-stagger (one-shot prologue offset 위주)
//   sleep_ns>0 && stagger_ns=0  → uniform throttle (모든 SM 같은 sleep, 종전 동작)
